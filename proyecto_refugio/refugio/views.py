...
from .models import Animal, SolicitudAdopcion, Donacion
...
def lista_animales(request):
    ...
    busqueda = request.GET.get('q') # NUEVO: Capturamos el texto del buscador
    ...
    if busqueda:
        # Filtramos por nombre que contenga el texto (ignorando mayúsculas/minúsculas)
        animales = animales.filter(nombre__icontains=busqueda)

    return render(request, 'refugio/lista.html', {'animales': animales})
...
def donar_animal(request, animal_id):
    ...
    if animal.estado != 'REHAB':
        messages.error(request, 'Solo puedes donar para el tratamiento de animales en Rehabilitación.')
        return redirect('detalle_animal', animal_id=animal.id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            donacion.animal = animal
            donacion.save()
            messages.success(request, f'¡Gracias por tu apoyo! Has donado {donacion.cantidad}€ para ayudar a {animal.nombre}.')
            return redirect('mi_perfil')
    else:
        form = DonacionForm()
